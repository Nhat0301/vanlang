...
jQuery(function($) {
    ...
    if (stickyHeaderVar == '1') {
		...
		function checkHolderOffset() {
		  return pageHeight <= window.scrollY;
		}
		var showSticky = function() {
			spHeader.classList.toggle('sticky', checkHolderOffset());
			if (spHeader.classList.contains("sticky")) {
			  document.querySelector(".sp-sticky-logo").style.display = 'block';
			} else {
			  document.querySelector(".sp-sticky-logo").style.display = 'none';
			}
		}
		function tryCheck() {
